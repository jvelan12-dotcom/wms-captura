...
export default async function handler(req) {
  ...
  try {
    ...
    if (data.error) {
      return new Response(JSON.stringify({ error: 'Gemini: ' + data.error.message + ' (code: ' + data.error.code + ')' }), { status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }});
    }
